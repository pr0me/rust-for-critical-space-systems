...
/// Handle incoming CSP v2.0 CAN frames
#[no_mangle]
pub extern "C" fn csp_can2_rx(
    ...
    let ifdata = curr_iface.interface_data as *mut libcsp_ffi::csp_can_interface_data_t;
    let mut packet = unsafe {
        libcsp_ffi::csp_can_pbuf_find(ifdata, id, libcsp_ffi::CFP2_ID_CONN_MASK, task_woken)
    };

    if packet.is_null() {
        if id & (libcsp_ffi::CFP2_ID_CONN_MASK << libcsp_ffi::CFP2_BEGIN_OFFSET) != 0 {
            packet = unsafe { libcsp_ffi::csp_can_pbuf_new(ifdata, id, task_woken) };
            if packet.is_null() {
                curr_iface.rx_error += 1;
                return CspError::Nomem;
            }
    ...
    let curr_packet = unsafe { &mut *packet };

    if id & (libcsp_ffi::CFP2_BEGIN_MASK << libcsp_ffi::CFP2_BEGIN_OFFSET) != 0 {
        if data_length < 4 {
            // Something something dbg errno
            curr_iface.rx_error += 1;
            unsafe { libcsp_ffi::csp_can_pbuf_free(ifdata, packet, 1, task_woken); }
            return CspError::Inval;
        }

        curr_iface.frame += 1;
        unsafe { csp_id_setup_rx(packet); }
        let first_two = id >> CFP2_DST_OFFSET as u16;

        #[cfg(target_endian = "little")]
        let first_two = u16::from_le(first_two);

        let frame_begin = unsafe {
            curr_packet.__bindgen_anon_1.__bindgen_anon_2.frame_begin
        };

        let frame_length = unsafe {
            curr_packet.__bindgen_anon_1.__bindgen_anon_2.frame_length
        };

        /*
        let mut curr_frame = unsafe {
            if !frame_begin.is_null() {
                core::slice::from_raw_parts(frame_begin, frame_length as usize)
            } else {
                return CspError::Inval;
            }
        }.as_mut();
